#! /bin/bash
# Copyright (c) 2026 Besher Mohammad Saleh Hassan. Licensed under the MIT License.
...
allocator=${ALLOCATOR:-input_parameter_zero}         
...
GRPO_UPDATE_INTERVAL=${GRPO_UPDATE_INTERVAL:-10}  
GRPO_GROUP_SIZE=${GRPO_GROUP_SIZE:-3}               
GRPO_MICROVAL_SIZE=${GRPO_MICROVAL_SIZE:-16}        
GRPO_POLICY_LR=${GRPO_POLICY_LR:-0.05}
GRPO_KL_COEFF=${GRPO_KL_COEFF:-0.05}
GRPO_ENTROPY_BONUS=${GRPO_ENTROPY_BONUS:-0.01}

GRPO_MAX_DELTA_P=${GRPO_MAX_DELTA_P:-0.10}          
...
run_job "arabic_qa" "--english_qa --arabic_qa"

run_job "chinese_qa" "--english_qa --chinese_qa"
