...
for seed in "${SEEDS[@]}"; do
  for delete_percent in "${AR_DELETE_PERCENTS[@]}"; do
    run_job "arabic_qa" "--english_qa --arabic_qa" "merge_eng_ar_qa_del" "$delete_percent" "$seed"
  done
done

for seed in "${SEEDS[@]}"; do
  for delete_percent in "${CH_DELETE_PERCENTS[@]}"; do
    run_job "chinese_aq" "--english_qa --chinese_aq" "merge_eng_ch_qa_del" "$delete_percent" "$seed"
  done
done
