...
for seed in "${SEEDS[@]}"; do
  for delete_percent in "${AR_DELETE_PERCENTS[@]}"; do
    generate_with_merge \
      "arabic_summary" \
      "$MERGE_ROOT/merge_eng_ar_summary_del/del${delete_percent}_seed${seed}" \
      "$output_root/arabic_summary_merge/del${delete_percent}_seed${seed}"
  done

  for delete_percent in "${CH_DELETE_PERCENTS[@]}"; do
    generate_with_merge \
      "chinese_summary" \
      "$MERGE_ROOT/merge_eng_ch_summary_del/del${delete_percent}_seed${seed}" \
      "$output_root/chinese_summary_merge/del${delete_percent}_seed${seed}"
  done
done
